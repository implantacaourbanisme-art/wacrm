...
describe('parseSyncBody', () => {
  it('accepts a batch and normalizes fields', () => {
    ...
    expect(r).toEqual({
      ok: true,
      items: [
        { phone: '+558296004382', name: 'Ana', email: 'A@B.com', customFields: { 'CPF/CNPJ': '52998224725' } },
      ],
    })
  })
  it('drops an invalid CPF/CNPJ but keeps the contact', () => {
    ...
    expect(r).toEqual({
      ok: true,
      items: [{ phone: '5582', name: null, email: null, customFields: {} }],
    })
  })
  it('ignores empty custom field values', () => {
    const r = parseSyncBody({ contacts: [{ phone: '5582', custom_fields: { Outro: '  ' } }] })
    expect(r).toEqual({ ok: true, items: [{ phone: '5582', name: null, email: null, customFields: {} }] })
  })
  it('rejects a bad body, an empty or oversized batch, and an item without phone', () => {
    expect(parseSyncBody(null).ok).toBe(false)
    expect(parseSyncBody({}).ok).toBe(false)
    expect(parseSyncBody({ contacts: [] }).ok).toBe(false)
    expect(parseSyncBody({ contacts: [{ name: 'x' }] }).ok).toBe(false)
