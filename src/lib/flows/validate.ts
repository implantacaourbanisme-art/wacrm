...
export function validateFlowForActivation(
  ...
  if (nodes.length === 0) {
    issues.push({
      severity: "error",
      scope: "flow",
      message: "O fluxo precisa de pelo menos um nó antes de ser ativado.",
    });
  }
    ...
    if (!keywords || keywords.length === 0) {
      ...
      if (blanks > 0) {
        issues.push({
          ...
          scope: "trigger",
          field: "trigger_config.keywords",
          message: `${blanks} ${blanks === 1 ? "palavra-chave está em branco e não vai" : "palavras-chave estão em branco e não vão"} corresponder a nada.`,
        });
      }
    }
  }
    ...
    case "send_list": {
      ...
      if (!cfg.button_label?.trim()) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "button_label",
          message: "Enviar lista precisa de um rótulo de botão (o texto que abre a lista).",
        });
      }
      ...
      if (totalRows > INTERACTIVE_LIMITS.maxListRowsTotal) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "sections",
          message: `Enviar lista permite no máximo ${INTERACTIVE_LIMITS.maxListRowsTotal} linhas no total, somando todas as seções.`,
        });
      }
      const seenIds = new Set<string>();
        ...
        rows.forEach((row, ri) => {
          ...
          if (!row.reply_id?.trim()) {
            ...
            issues.push({
              ...
              node_key: node.node_key,
              field: `${field}.reply_id`,
              message: `Id de linha da lista duplicado: "${row.reply_id}".`,
            });
          }
    ...
    case "collect_input": {
      ...
      if (!cfg.var_key?.trim()) {
        ...
        issues.push({
          ...
          node_key: node.node_key,
          field: "var_key",
          message: `A var_key "${cfg.var_key}" deve ter apenas letras, números e sublinhado, e começar com letra ou sublinhado.`,
        });
      }
      if (!cfg.next_node_key) {
        issues.push({
    ...
    case "condition": {
      ...
      if (
        ...
        issues.push({
          ...
          node_key: node.node_key,
          field: "value",
          message: `O operador "${cfg.operator}" normalmente espera um valor de comparação — um valor vazio só corresponde a assuntos vazios.`,
        });
      }
      for (const branch of ["true_next", "false_next"] as const) {
        ...
        if (!key) {
          ...
          issues.push({
            ...
            node_key: node.node_key,
            field: branch,
            message: `O ramo "${branch}" da condição aponta para um nó inexistente: "${key}".`,
          });
        }
      }
      break;
    ...
    case "set_tag": {
      ...
      if (!cfg.mode || !["add", "remove"].includes(cfg.mode)) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "mode",
          message: "Definir etiqueta precisa de um modo (adicionar ou remover).",
        });
      }
      if (!cfg.tag_id) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "tag_id",
          message: "Definir etiqueta precisa de uma etiqueta para aplicar.",
        });
      }
      if (!cfg.next_node_key) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "next_node_key",
          message: "Definir etiqueta precisa apontar para um próximo nó.",
        });
      } else if (!knownKeys.has(cfg.next_node_key)) {
        issues.push({
          severity: "error",
          ...
          node_key: node.node_key,
          field: "next_node_key",
          message: `Definir etiqueta aponta para um nó inexistente: "${cfg.next_node_key}".`,
        });
      }
      break;
    }
