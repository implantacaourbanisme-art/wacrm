...
const SPEC_DEFAULT_STAGES = [
  { name: "Contato inicial", color: "#3b82f6", position: 0 }, // blue
  { name: "Visita agendada", color: "#eab308", position: 1 }, // yellow
  { name: "Proposta", color: "#f97316", position: 2 }, // orange
  { name: "Negociação", color: "#8b5cf6", position: 3 }, // purple
  { name: "Contrato", color: "#22c55e", position: 4 }, // green
];
...
export default function PipelinesPage() {
  ...
  const seedDefaultPipeline = useCallback(async (): Promise<Pipeline | null> => {
    ...
    const { data: pipeline, error } = await supabase
      .from("pipelines")
      .insert({ user_id: user.id, account_id: accountId, name: "Vendas" })
      .select()
      .single();
